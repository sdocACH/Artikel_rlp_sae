...
The local density on cluster level $Y_{c}(x)$, which in our case the timber volume per hectare, is then defined as the average of the individual sample plot densities $Y(x_l)$:
...
error(\hat{Y})_{[\%]}=\frac{\sqrt{\hat{\var}(\hat{Y})}}{\hat{Y}}*100
\end{equation}
...
\begin{table}[H]
	\begin{center}
		\caption{Sample size for each phase in entire study area. $n_{\{1,2\},plots}$: number of plots. $n_{\{1,2\}}$: number of clusters. TSPEC: tree species information.}
		\vspace{0.2cm}
		\label{tab:ssize}
		{\small %
